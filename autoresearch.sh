...
# Run build benchmark (50 iterations, 50 products)
BUILD_OUT=$(uv run bench_build.py --iterations 50 --num-products 50 2>&1)
BUILD_MS=$(echo "$BUILD_OUT" | grep 'shifthtml ' | head -1 | grep -oE 'median=[[:space:]]*[0-9.]+' | grep -oE '[0-9.]+')

# Run render benchmark (50 iterations, 50 products)
RENDER_OUT=$(uv run bench_render.py --iterations 50 --num-products 50 2>&1)
